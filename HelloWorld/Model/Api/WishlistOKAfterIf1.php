...
class Wishlist implements PostManagementInterface
{
    ...
    public function getWishlistDetailForCustomer($customerId, SearchCriteriaInterface $searchCriteria)
    {
        ...
        foreach ($wishlist_collection as $item)
        {
            ...
            $wishlistItemIdList[$parentId] = array($wishlistItemId, $qty); 
            if ($parentId !== $childId){
                $wishlistItemIdList[$parentId] = array($wishlistItemId, $qty, $childId);                
            }
        }
        ...
        foreach ($product as $productItem) {
            ...
            if (count($wishlistItemIdList[$productItemId]) == 3) {                            
                $childId = $wishlistItemIdList[$productItemId][2];                   
                $_configProduct = $this->_productRepository->getById($productItemId);
                $attributeArray = $_configProduct->getExtensionAttributes()->getConfigurableProductOptions();                
                ...
                foreach ($attributeArray as $attributeList) {                 
                    ...
                    foreach($rows as $row){
                        ...
                        $productAttributes = $this->wishlistDataFactory->create();                            
                        ...
                        $productItem->setOptions($setProductAttributes);                       
                    }
                }
                ...
                $productItem->setWishlistImagePath($this->imageHelper->init($productItem, 'product_page_image_small')
                ->setImageFile($productItem->getSmallImage())->resize(194)->getUrl()); 

                // _configProduct
                
                // $customAttribute->setValue($this->imageHelper->init($product, 'product_page_image_small')
                // ->setImageFile($product->getSmallImage())->resize(512)->getUrl());

                // $data->setFile($this->imageHelper->init($item, 'product_page_image_small')
                // ->setImageFile($item->getSmallImage())->resize(194)->getUrl());
        ...
        $searchResult->setCurrentPage($searchCriteria->getCurrentPage()); 
        return $searchResult;
    }
