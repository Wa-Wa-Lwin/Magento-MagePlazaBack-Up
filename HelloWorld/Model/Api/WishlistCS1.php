...
class Wishlist implements PostManagementInterface
{
     ...
     * @param string $customerId
    * @return \MIT\Product\Api\Data\WishlistProductResultListInterface
     */
    public function getWishlistDetailForCustomer($customerId, SearchCriteriaInterface $searchCriteria)
    {
        ...
        foreach ($wishlist_collection as $item)
        {
            ...
            echo("parent is ".$parentId."_ child is".$childId."_ |");

         //   $productItemId = $parentId;

            $wishlistItemId = $item->getWishlistItemId(); //OK
            $qty = round($item->getQty()); //OK
       ...
       $productList = $this->productApiInterface->getList(0,$searchCriteria);
        ...
        foreach ($product as $productItem) {
            ...
            $setWishlistItemId = $wishlistItemIdList[$productItemId][0];
               ...
               // var_dump(" setQty =".$setQty);

            echo("count === ".count($wishlistItemIdList[$productItemId])); 

            echo("productItemId === ".$productItemId); 

            // if ($childId != $productItemId) {       
            if (count($wishlistItemIdList[$productItemId]) == 3) {

                // echo ("childId != productItemId is working"); 

               

                // var_dump(" childId =".$childId);

                // var_dump(" productItemId =".$productItemId);
                ...
                echo ("Nooo childId != productItemId is NOT  working"); 
                $setWishlistItemId = $wishlistItemIdList[$productItemId][0];
                $setQty = $wishlistItemIdList[$productItemId][1];
