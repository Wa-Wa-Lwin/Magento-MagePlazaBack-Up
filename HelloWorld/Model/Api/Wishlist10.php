    ...
    public function getWishlistDetailForCustomer($customerId, SearchCriteriaInterface $searchCriteria)
    {
      ...
      //  $childIds = []; 
        ...
        foreach ($wishlist_collection as $item) {
            ...
            // $parentId = $item->getProduct()->getProductId();
            //  $item->getWishlistItemId() getWishlistId
            // $item->getProductId()
            ...
            // $setWishlistItemId = null; // Initialize the variable
          ...
          //  $childId = $childProduct->getId();
            if ($childId!=$parentId) {
                ...
                //$childId = 123; // Replace with the actual child product ID

                $attributeCode = 'size'; // Replace with the desired attribute code

              //  $table1 = $connection->getTableName('catalog_product_entity_int');
                $table2 = $connection->getTableName('eav_attribute');

                // Find the attribute ID based on the attribute code
                $selectAttributeId = $connection->select()
                    ->from($table2, ['attribute_id'])
                ...
                {
                    echo("oknow for attributeId?".$attributeId);
                ...
                //$attributeArray = $item->getExtensionAttributes()->getConfigurableProductOptions();
                ...
                //foreach ($attributeArray as $attributeList) {
                    ...
                    $select = $connection->select()
                        ...
                        ->where('eav_attribute.attribute_id = ?', $attribute_id)
                    ...
                    foreach ($rows as $row) {
                        ...
                        echo ("option_value".$setOptionValue);
                        echo ("option_code".$setOptionCode);
